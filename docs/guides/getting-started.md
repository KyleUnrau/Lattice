...
**Accounts:** `cash`, `inventory`, `wallet`, `openingBalance`, `exchangeExpense`, `capitalGains`, `exchangePositions`
...
> phase1()      // Exchange 500 CAD → 375 USD (forward exchange)
> phase2()      // Exchange 375 USD → 1500 Oranges (forward exchange)
...
// 10000n  (100.00 CAD gain in smallest-unit bigint)

> exchangePositions.getRootBalances(ledger.transactions)
// Shows any remaining open exchange positions (should be empty after all phases)
```
...
> dump(engine.compute(utxo, someQuantity))
```
